...
def test_archive_contains_doc_kml(actual_result):
    assert actual_result.archive_entries == ["doc.kml"]


def test_namespace_and_verified_geometry_inventory(actual_result):
...
def test_unicode_names_are_preserved(actual_result):
    assert actual_result.areas[0].area_name == "Уудавын булаг"
    assert actual_result.areas[-1].area_name == "Хөх хадны худаг"


def test_polygon_versus_point_classification(actual_result):
...
def test_expected_codes_and_no_validation_errors(actual_result):
...
def test_missing_doc_kml_is_rejected(tmp_path: Path):
...
def test_wrong_kml_namespace_is_rejected(tmp_path: Path):
...
def test_manifest_geojson_and_individual_kml_generation(actual_result, tmp_path: Path):
